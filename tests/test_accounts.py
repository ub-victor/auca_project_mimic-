...
TEST_PASSWORD = 'T3st-P@ssw0rd-AUCA'
...
class SignupViewTest(TestCase):
    ...
    def test_signup_password_mismatch(self):
        self.client.post(self.url, {
            ...
            'password1':  TEST_PASSWORD,
            'password2':  TEST_PASSWORD + 'wrong',
        })
        self.assertEqual(User.objects.filter(email='test2@auca.ac.rw').count(), 0)

    def test_signup_duplicate_email(self):
        User.objects.create_user(username='dup@auca.ac.rw', email='dup@auca.ac.rw', password=TEST_PASSWORD)
        self.client.post(self.url, {
...
class LoginViewTest(TestCase):
    ...
    def test_authenticated_user_redirected_from_login(self):
        self.client.login(username='login@auca.ac.rw', password=TEST_PASSWORD)
        response = self.client.get(self.url)
        self.assertRedirects(response, reverse('dashboard'))
...
class ProfileViewTest(TestCase):
    def setUp(self):
        ...
        self.client.login(username='profile@auca.ac.rw', password=TEST_PASSWORD)
...
class DecoratorTest(TestCase):
    ...
    def _make_user(self, role, email):
        return User.objects.create_user(username=email, email=email, password=TEST_PASSWORD, role=role)

    def test_student_required_blocks_lecturer(self):
        self._make_user('lecturer', 'lec@auca.ac.rw')
        self.client.login(username='lec@auca.ac.rw', password=TEST_PASSWORD)
    ...
    def test_staff_required_blocks_student(self):
        ...
        self.client.login(username='stu@auca.ac.rw', password=TEST_PASSWORD)
    ...
    def test_lecturer_required_allows_student(self):
        ...
        self.client.login(username='stu2@auca.ac.rw', password=TEST_PASSWORD)
